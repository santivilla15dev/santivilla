...
export async function POST(req: Request, { params }: Params) {
  ...
  try {
    ...
    let body: { message?: string; lang?: string; editToken?: string };
    try {
    ...
    if (!conceptEditTokenOk(concept, body.editToken)) {
      return NextResponse.json(
        {
          error: "FORBIDDEN",
          message: "Solo quien generó el concepto puede chatear con él.",
        },
        { status: 403 },
      );
    }
    ...
    console.error("[concepto chat]", err);
    return NextResponse.json(
      { error: "INTERNAL", message: "El agente no pudo responder." },
      { status: 500 },
    );
  }
}
